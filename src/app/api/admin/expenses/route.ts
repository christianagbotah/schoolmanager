...
// GET /api/admin/expenses - List expenses with search, filtering, and summary
  ...
  try {
    ...
    if (search) {
      where.OR = [
        ...
        { expense_category: { expense_category_name: { contains: search } } },
      ]
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ expense, message: 'Expense created successfully' }, { status: 201 })
  } catch (error) {
...
// PUT /api/admin/expenses/[id] - handled by the [id] route
// DELETE /api/admin/expenses/[id] - handled by the [id] route
