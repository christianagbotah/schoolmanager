...
export default function ParentResultsPage() {
  ...
  // ─── Fetch exams ───────────────────────────────────────────
  useEffect(() => {
    if (!selectedStudent) { setExams([]); return; }
  ...
  const fetchMarks = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/marks?student_id=${selectedStudent}&exam_id=${selectedExam}&limit=200`);
              ...
              <CardHeader>
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="text-base font-semibold">
                    {exams.find(e => e.exam_id === parseInt(selectedExam))?.name || "Exam"} — {students.find(s => s.student_id === parseInt(selectedStudent))?.name || "Student"}
                  </CardTitle>
                </div>
              </CardHeader>
          ...
          <Card className="gap-4">
            <CardContent className="py-16">
              <div className="text-center">
                ...
                <h3 className="text-lg font-medium text-slate-600">Select a child to view results</h3>
