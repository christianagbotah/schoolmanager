...
import {
  Users, GraduationCap, DollarSign, TrendingUp, Printer, Download,
  BarChart3, School, Calendar, Wallet, ChevronRight,
} from 'lucide-react';
...
function AnnualReportSkeleton() {
  return (
    <div className="space-y-6">
      ...
      <Card className="border-slate-200/60">
        <CardContent className="p-6"><Skeleton className="h-52 w-full" /></CardContent>
      </Card>
    </div>
  );
}
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 border-b border-slate-100 print:hidden">
              ...
              <div className="flex items-center gap-2">
                <Select value={year} onValueChange={setYear}>
                  <SelectTrigger className="w-32 min-h-[44px] bg-slate-50 border-slate-200 focus:bg-white"><SelectValue /></SelectTrigger>
            ...
            <div className="hidden print:block text-center mb-6">
              <h1 className="text-xl font-bold">Annual Comprehensive Report &mdash; {year}</h1>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <Card className="border-l-4 border-l-emerald-500 hover:shadow-sm transition-all hover:-translate-y-0.5">
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xl font-bold text-slate-900 tabular-nums">{summary?.totalStudents?.toLocaleString() || 0}</p>
                  </div>
                </CardContent>
              </Card>
              <Card className="border-l-4 border-l-sky-500 hover:shadow-sm transition-all hover:-translate-y-0.5">
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-lg font-bold text-slate-900 tabular-nums">{fmt(summary?.totalRevenue || 0)}</p>
                  </div>
                </CardContent>
              </Card>
              <Card className="border-l-4 border-l-amber-500 hover:shadow-sm transition-all hover:-translate-y-0.5">
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-lg font-bold text-slate-900 tabular-nums">{fmt(summary?.totalExpenses || 0)}</p>
                  </div>
                </CardContent>
              </Card>
              <Card className="border-l-4 border-l-violet-500 hover:shadow-sm transition-all hover:-translate-y-0.5">
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="text-[10px] text-slate-400 uppercase tracking-wider font-medium">Net Profit</p>
                    <p className={`text-lg font-bold tabular-nums ${(summary?.netProfit || 0) >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>{fmt(summary?.netProfit || 0)}</p>
                  </div>
                </CardContent>
              </Card>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <Card className="border-slate-200/60 hover:shadow-sm transition-all">
                <CardContent className="p-4 text-center">
                  <School className="w-5 h-5 text-slate-400 mx-auto mb-1" />
                  <p className="text-[10px] text-slate-400 uppercase tracking-wider font-medium">Total Classes</p>
                  <p className="text-lg font-bold text-slate-900">{summary?.totalClasses || 0}</p>
                </CardContent>
              </Card>
              <Card className="border-slate-200/60 hover:shadow-sm transition-all">
                <CardContent className="p-4 text-center">
                  <GraduationCap className="w-5 h-5 text-violet-400 mx-auto mb-1" />
                  <p className="text-[10px] text-slate-400 uppercase tracking-wider font-medium">Total Teachers</p>
                  <p className="text-lg font-bold text-slate-900">{summary?.totalTeachers || 0}</p>
                </CardContent>
              </Card>
              <Card className="border-slate-200/60 hover:shadow-sm transition-all">
                <CardContent className="p-4 text-center">
                  ...
                  <p className="text-lg font-bold text-amber-600">{fmt(summary?.totalOutstanding || 0)}</p>
                </CardContent>
              </Card>
              <Card className="border-slate-200/60 hover:shadow-sm transition-all">
                <CardContent className="p-4 text-center">
                  <BarChart3 className="w-5 h-5 text-slate-400 mx-auto mb-1" />
                  <p className="text-[10px] text-slate-400 uppercase tracking-wider font-medium">School Average</p>
                  <p className="text-lg font-bold text-slate-900">{summary?.overallAverage || 0}</p>
                </CardContent>
              </Card>
            </div>
              ...
              <CardContent className="p-4">
                {enrollmentTrend.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 text-slate-400">
                    ...
                    <p className="font-medium text-slate-900 mb-1">No enrollment data</p>
                    <p className="text-sm text-slate-500">No student admissions recorded for {year}</p>
                  </div>
                ) : (
              ...
              <CardContent className="p-4">
                {financialChart.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 text-slate-400">
                    ...
                    <p className="font-medium text-slate-900 mb-1">No financial data</p>
                    <p className="text-sm text-slate-500">No income or expense records for {year}</p>
                  </div>
                ) : (
                  <div>
                    <div className="flex items-end gap-1 h-48">
                ...
                <CardContent className="p-0">
                  {academicPerformance.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-12 text-slate-400">
                      ...
                      <p className="font-medium text-slate-900 mb-1">No academic data</p>
                      <p className="text-sm text-slate-500">No class performance records available</p>
                    </div>
                  ) : (
                    <>
                      {/* Desktop Table */}
                ...
                <CardContent className="p-4">
                  {Object.keys(gradeDistribution).length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-12 text-slate-400">
                      ...
                      <p className="font-medium text-slate-900 mb-1">No grade data</p>
                      <p className="text-sm text-slate-500">No grade distribution records available</p>
                    </div>
                  ) : (
