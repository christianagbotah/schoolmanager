...
export default function StudentNoticesPage() {
  ...
  if (isLoading) {
    return (
      <DashboardLayout>
        <div className="space-y-6"><Skeleton className="h-8 w-48" /><Skeleton className="h-12 w-full" /><div className="space-y-3">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-20 w-full" />)}</div></div>
      </DashboardLayout>
    );
  }
                ...
                <CardContent className="p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2"><Pin className="w-4 h-4 text-amber-500" /><h4 className="font-semibold text-slate-900">{notice.title}</h4></div>
                      <p className="text-sm text-slate-500 mt-1 line-clamp-2">{notice.notice}</p>
                      {notice.timestamp && <span className="text-xs text-slate-400 mt-2 flex items-center gap-1"><Calendar className="w-3 h-3" />{format(new Date(notice.timestamp), "MMM d, yyyy")}</span>}
                    </div>
                    <Button variant="ghost" size="sm" onClick={() => setSelectedNotice(notice)} className="min-w-[44px] min-h-[44px]"><Eye className="w-4 h-4" /></Button>
                  </div>
                </CardContent>
              </Card>
            ))}
                  ...
                  <div key={notice.id} className="p-4 rounded-lg border border-slate-100 hover:bg-slate-50 transition-colors cursor-pointer" onClick={() => setSelectedNotice(notice)}>
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        {notice.timestamp && <span className="text-[10px] text-slate-400 mt-1">{format(new Date(notice.timestamp), "MMM d, yyyy")}</span>}
                      </div>
          ...
          <DialogContent className="max-w-lg">
            ...
            {selectedNotice && (
              <div className="space-y-4">
                {selectedNotice.timestamp && <p className="text-sm text-slate-400 flex items-center gap-1"><Calendar className="w-4 h-4" />{format(new Date(selectedNotice.timestamp), "EEEE, MMMM d, yyyy")}</p>}
                <div className="text-sm text-slate-700 whitespace-pre-wrap leading-relaxed">{selectedNotice.notice}</div>
              </div>
            )}
          </DialogContent>
        </Dialog>
